...
class VideoCombinePlus:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "hidden": {
                # Keep these available for workflows that pass them, but don't
                # show them as visible input sockets on the node.
                "vae": "VAE",
    ...
    def combine_video(
        ...
        try:
            ...
            frames_to_video(frames=frames, fps=frame_rate, output_path=out_path, audio_path=audio_path, volume=volume, fmt=format, pingpong=pingpong)
        finally:
